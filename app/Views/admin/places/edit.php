...
<div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Tempat Kuliner</h1>
        <p class="text-sm text-slate-500 mt-1">Mengubah data <?= esc($place['name']) ?></p>
    </div>

    <div class="bg-white/80 backdrop-blur-sm rounded-2xl ring-1 ring-slate-900/5 shadow-sm p-6 sm:p-8 max-w-3xl card-hover">
        <form action="/admin/places/<?= $place['id'] ?>/update" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="mb-5">
                ...
                <select name="category_id" id="category_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm appearance-none cursor-pointer">
                    ...
                    <option value="<?= $cat['id'] ?>" <?= old('category_id', $place['category_id']) == $cat['id'] ? 'selected' : '' ?>><?= esc($cat['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-5">
                ...
                <div class="flex gap-2">
                    <input type="text" name="address" id="address" value="<?= old('address', $place['address']) ?>" required
                        class="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm">
            ...
            <div class="mb-5">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm resize-none"><?= old('description', $place['description'] ?? '') ?></textarea>
            </div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <label class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-50 ring-1 ring-slate-200 hover:ring-orange-300 hover:bg-orange-50/50 cursor-pointer transition-all duration-200 has-[:checked]:ring-orange-400 has-[:checked]:bg-orange-50 has-[:checked]:ring-2">
                        <input type="checkbox" name="tags[]" value="<?= $tag['id'] ?>"
                            <?= in_array($tag['slug'], $currentTags) ? 'checked' : '' ?>
            ...
            <div class="mb-5">
                ...
                <select name="status" id="status" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm appearance-none cursor-pointer">
                    <option value="approved" <?= old('status', $place['status'] ?? 'approved') === 'approved' ? 'selected' : '' ?>>Disetujui</option>
                    <option value="pending" <?= old('status', $place['status'] ?? '') === 'pending' ? 'selected' : '' ?>>Menunggu</option>
                    <option value="rejected" <?= old('status', $place['status'] ?? '') === 'rejected' ? 'selected' : '' ?>>Ditolak</option>
                </select>
            </div>
