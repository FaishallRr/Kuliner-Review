...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center gap-3.5 mb-6">
        <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center shadow-sm shrink-0">
            <svg class="w-5.5 h-5.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
        </div>
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Edit <?= esc($place['name']) ?></h1>
            <p class="text-sm text-slate-500">Perbarui informasi tempat kuliner</p>
        </div>
    </div>
    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-2xl ring-1 ring-slate-900/5 shadow-sm p-6 sm:p-8 animate-fade-in card-hover">
        <div class="flex items-start gap-3 mb-6 p-4 rounded-xl bg-amber-50/80 ring-1 ring-amber-200/50">
            ...
            <p class="text-sm text-amber-700">Perubahan akan mengembalikan status ke <strong>Menunggu</strong> untuk ditinjau ulang admin.</p>
        </div>

        <form action="/places/<?= $place['id'] ?>/update" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="mb-5">
                ...
                <select name="category_id" id="category_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm appearance-none cursor-pointer">
                    ...
                    <option value="<?= $cat['id'] ?>" <?= $place['category_id'] == $cat['id'] ? 'selected' : '' ?>><?= esc($cat['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-5">
                ...
                <div class="flex gap-2">
                    <input type="text" name="address" id="address" value="<?= old('address', $place['address']) ?>" required
                        class="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm placeholder:text-slate-400">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-2">
                <div>
                    ...
                    <input type="text" name="latitude" id="latitude" value="<?= old('latitude', $place['latitude']) ?>"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm">
                </div>
                <div>
                    <label for="longitude" class="block text-sm font-semibold text-slate-700 mb-1.5">Longitude</label>
                    <input type="text" name="longitude" id="longitude" value="<?= old('longitude', $place['longitude']) ?>"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm">
                </div>
            </div>
            ...
            <div class="mb-5">
                ...
                <div class="mb-3 inline-block">
                    <img src="/uploads/<?= esc($place['image']) ?>" alt="Foto saat ini" class="h-32 w-auto rounded-xl ring-1 ring-slate-900/5 shadow-sm">
                    <p class="text-xs text-slate-400 mt-1.5">Foto saat ini. Upload foto baru untuk mengganti.</p>
                </div>
                <?php endif; ?>
            ...
            <div class="mb-5">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-orange-500/20 focus:border-orange-400 focus:bg-white transition-all text-sm resize-none placeholder:text-slate-400"><?= old('description', $place['description']) ?></textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <label class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-50 ring-1 ring-slate-200 hover:ring-orange-300 hover:bg-orange-50/50 cursor-pointer transition-all duration-200 has-[:checked]:ring-orange-400 has-[:checked]:bg-orange-50 has-[:checked]:ring-2">
                        <input type="checkbox" name="tags[]" value="<?= $tag['id'] ?>" <?= in_array($tag['slug'], $currentTags) ? 'checked' : '' ?> class="rounded text-orange-600 focus:ring-orange-500">
